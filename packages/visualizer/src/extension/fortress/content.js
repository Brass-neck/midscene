...
const NodeType = {
  ...
  ElementExisting: 'ElementExistingNode', // 元素存在
...
async function pressKeyWithJs({ element = document, keys, pressTime }) {
  const details = {
    key: '',
    code: '',
    keyCode: '',
    ...
    for (const key of keys) {
      ...
      if (isEditable || isTextField) {
        ...
        if (key === 'Enter') {
          ...
          console.log('debug Enter1', element);
          ...
          console.log('debug Enter2', element);
        }
      }
...
async function pressKey({ ele, keys, pressTime = 0 }) {
  let element = document.activeElement;

  if (ele) {
    element = ele || element;
  }
  console.log('debug pressKey element', element);
  await pressKeyWithJs({
    keys,
    element,
    pressTime: Number.isNaN(+pressTime) ? 0 : Math.abs(+pressTime),
  });
...
const findAllNodeTypes = (node = document) => {
  let map = {};
  if (node.nodeType === 1) {
  ...
  for (let i = 0; i < children.length; i++) {
    //递归调用
    map = { ...map, ...findAllNodeTypes(children[i]) };
  }

  return map;
};

const findElementByText = (text = '', parentNode = document) => {
  if (!text) return null;
  let node = null; // 最终返回的HTML Node 节点数组
  const domNodeTypesMap = findAllNodeTypes(parentNode);
  const domNodeTypes = Object.keys(domNodeTypesMap).filter(
    (item) => !DISABLE_FIND_NODE_TYPES.includes(String(item).toUpperCase()),
  );
  const domNodes = domNodeTypes.reduce((prev, next) => {
    prev = [...prev, ...document.querySelectorAll(next)];
    return prev;
  }, []);
...
const findElement = async (selector, textSelector, maxFindTimes, isOversea) => {
  if (!maxFindTimes || (!selector && !textSelector)) {
    return Promise.resolve(
      `${isOversea ? 'Not find Element' : '未找到元素'}：` +
        (selector || '') +
        (textSelector || ''),
    );
  }
  ...
  return new Promise((resolve, reject) => {
    ...
    if (textSelector) {
      ele = findElementByText(textSelector, ele || document);
    }

    if (ele) {
      console.log('debug findElement', ele);
      resolve(ele);
    } else {
      if (maxFindTimes > 0) {
        console.log('debug notFindElement', ele, maxFindTimes);
        setTimeout(async () => {
          resolve(
            findElement(selector, textSelector, --maxFindTimes, isOversea),
          );
        }, 1000);
      } else {
        console.log('debug notFindElement', maxFindTimes);
        reject(`${isOversea ? 'Not find Element' : '未找到元素'}`);
      }
    }
  });
};

const simulateEvent = async (ele, eventType) => {
  switch (eventType) {
    case SimulateEventType.CLICK:
      return new Promise((resolve, reject) => {
        setTimeout(() => {
          let evt = new MouseEvent(SimulateEventType.CLICK, {
            bubbles: true,
            cancelable: true,
            view: window,
          });
      ...
      break;
    case SimulateEventType.MOUSE_DOWN:
      return new Promise((resolve, reject) => {
        setTimeout(() => {
          let evt = new MouseEvent(SimulateEventType.MOUSE_DOWN, {
            bubbles: true,
            cancelable: true,
            view: window,
          });
      ...
      break;
    case SimulateEventType.MOUSE_OVER:
      return new Promise((resolve, reject) => {
        setTimeout(() => {
          let evt = new MouseEvent(SimulateEventType.MOUSE_OVER, {
            bubbles: true,
            cancelable: true,
            view: window,
          });
      ...
      break;
    case SimulateEventType.KEYBOARD_ENTER:
      // return new Promise((resolve, reject) => {
      //   setTimeout(() => {
      //     let evt = new MouseEvent(SimulateEventType.MOUSE_DOWN, {
      //       bubbles: true,
  ...
  console.log('debug simulateEvent', ele, eventType);
  // if (canceled) {
...
const excuteNode = async (node, index, uidl, tabId, extra) => {
  ...
  await new Promise(async (resolve, reject) => {
    try {
      ...
      switch (type) {
        case NodeType.StartNode:
          ...
          if (formData?.accessPartId) {
            ...
            if (
              !isOversea &&
              (!formData.accessPartType || formData.accessPartType === 'united')
            ) {
              const isTenant =
                findElementByText('租户视角') &&
                document.querySelector('.united-switch-checked-text ~ input');
              if (isTenant) {
                const switchEle = document.querySelector(
                  '.united-switch-checked-text ~ input',
                );
                console.log('debug switchEle ele', switchEle);
                switchEle.click();
              }
            }
            ...
            switch (formData.accessPartType) {
              case 'session_manage':
                // ele = await findElement('.semi-icon-globe', undefined, defaultMaxFindTimes(isOversea), isOversea);
                // if (typeof ele === 'string') {
                //   excuteRecord.errors.push({ node, errorMsg: ele });
                //   break;
                // } else {
                //   await simulateEvent(ele?.parentElement, SimulateEventType.MOUSE_OVER);
                // }
                break;
              default:
                ele = await findElement(
                ...
                if (typeof ele === 'string') {
                  ...
                  await simulateEvent(
                    ele?.parentElement,
                    SimulateEventType.CLICK,
                  );
                  await simulateEvent(
                    ele?.parentElement,
                    SimulateEventType.MOUSE_DOWN,
                  );
                }
            }
            ...
            switch (formData.accessPartType) {
              case 'session_manage':
                // ele = await findElement('.hiui-pro-access-party-select--list-item-text', accessPartyName, defaultMaxFindTimes(isOversea), isOversea);
                break;
              default:
                ele = await findElement(
                  'ul[role="menu"]:first-child',
                  accessPartyName,
                  defaultMaxFindTimes(isOversea),
                  isOversea,
                );
        ...
        case NodeType.ElementHover:
          ...
          if (typeof ele === 'string') {
            ...
            await simulateEvent(ele, SimulateEventType.MOUSE_OVER);
          }
          excuteRecord.passCaseNum++;
          break;
        case NodeType.ElementExisting:
          ele = await findElement(
            formData.selector,
            formData.textSelector,
            defaultMaxFindTimes(isOversea),
        ...
        case NodeType.HttpRequest:
          const headers = (formData?.httpHeaders || []).reduce((prev, next) => {
            prev[next.key] = next.value?.includes("'")
              ? String(next.value).replace(/\'/g, '')
              : Number(next.value);
            return prev;
          }, {});
          const body = (formData?.httpBody || []).reduce((prev, next) => {
            prev[next.key] = next.value?.includes("'")
              ? String(next.value).replace(/\'/g, '')
              : Number(next.value);
            return prev;
          }, {});
          await fetch(formData?.httpUrl, {
      ...
      excuteRecord.errors.push({ node, errorMsg: e.stack });
      resolve();
    } finally {
...
const isInFortress = () =>
  location.href.match('fortress') || location.href.match('localhost');

const getUidlByVariableInfo = (uidl, variableInfo = {}, extra) => {
  const { nodes } = uidl;
...
if (isInFortress()) {
  window.addEventListener('message', async (event) => {
    if (!event.data?.data?.uidl || event.data?.type !== 'fortress:preview') {
      return;
    }
...
(async () => {
  ...
  console.log('debug fortressContentLoaded first', cache.fortressContentLoaded);
  chrome.runtime.onMessage.addListener(
    async (
      { type, data: { uidl, tabId, excuteRecord: record } },
      sender,
      sendResponse,
    ) => {
